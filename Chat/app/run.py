...
from app.models import UserAccounts, Message
...
def generate_key(k):
    ...
    chooses = [random.choice(alphabet) for i in range(k)]
    return str.encode("".join(chooses))
...
def index():
    ...
    ).order_by(Message.CreateDate.asc()).all()

    mug_shot_title = UserAccounts.query.filter_by(UserName=user_id).first().MugShot
    messages_dic = {}
    messages_list = []
    for message in message_data[-4:]:
        key = str.encode(message.Key)
        des = DES.new(key, DES.MODE_ECB)
        message_decrypt = des.decrypt(message.Message.Messages).decode("utf-8")
        # message_decrypt = message.Message.Messages
        messages_dic['data'] = []
        messages_dic['UserName'] = message.Message.UserName
        messages_dic['Messages'] = message_decrypt
        # messages_dic['Messages'] = type(message_data)
...
def register():
    ...
    key = generate_key(8)
    new_account = UserAccounts(user_name=username, password=password, mugshot="default.jpg", key= key)
...
def send_inquiry(msg):
    ...
    key = str.encode(user.Key)
    des = DES.new(key, DES.MODE_ECB)
    message_pad = pad(msg['msg'])
    messages_encry = des.encrypt(message_pad)
    data_message = Message(
        user_name=user_id,
        # messages=msg['msg'],
        messages= messages_encry,
    ...
    emit('getInquiry', data, room=msg['room'],broadcast=True)
